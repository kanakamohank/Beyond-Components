"""Stage 3+4 (Phi-3 variant): multi-plane ensemble injection on
StereoSet for microsoft/Phi-3-mini-4k-instruct.

Mirrors stereoset_ensemble_eval_gemma.py but for Phi-3. Single-head
baseline: L28 H1 SV0 (paper primary for Phi-3 gender).

Outputs:
  helix_usage_validated/stereoset_ensemble_phi3.csv
  helix_usage_validated/stereoset_ensemble_phi3.txt
"""
...
SCAN_PATH = OUT_DIR / "stereoset_scan_phi3.jsonl"
...
def main():
    ...
    ap.add_argument("--out_prefix", default="stereoset_ensemble_phi3")
    args = ap.parse_args()
    ...
    # single-head paper primary for Phi-3 gender
    primary_L, primary_H = 28, 1
    primary_plane = (0, 2)
