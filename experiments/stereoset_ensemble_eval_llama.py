"""Stage 3+4 (Llama variant): multi-plane ensemble injection on
StereoSet for meta-llama/Llama-3.2-3B.

Single-head baseline: L26 H14 SV2 (paper primary for Llama gender).

Outputs:
  helix_usage_validated/stereoset_ensemble_llama.csv
  helix_usage_validated/stereoset_ensemble_llama.txt
"""
...
SCAN_PATH = OUT_DIR / "stereoset_scan_llama.jsonl"
...
def main():
    ...
    # Llama singular values are tiny (~1); default to larger alphas.
    ap.add_argument("--alphas", default="5.0,10.0,20.0")
    ap.add_argument("--domains", default="gender,race,profession,religion")
    ap.add_argument("--sanity_n", type=int, default=0)
    ap.add_argument("--out_prefix", default="stereoset_ensemble_llama")
    args = ap.parse_args()
    ...
    # single-head paper primary for Llama-3.2-3B gender
    primary_L, primary_H = 26, 14
    primary_plane = (0, 2)
