#!/usr/bin/env python3
"""
Arithmetic Circuit Scan + Unembed-Aligned Patching (v2 — multi-model)

Fixes over v1:
  BUG-1  compute_unembed_basis: centering was dim=0 (wrong). Fixed to dim=1.
...
Usage:
    python arithmetic_circuit_scan.py --device cpu
    python arithmetic_circuit_scan.py --device cpu --skip-scan --unembed-layers 28,29
    python arithmetic_circuit_scan.py --device cpu --no-format-removal
"""
...
def format_arithmetic_prompt(a: int, b: int, answer_str: str, prompt_format: str = "calculate") -> str:
    """
    Format a teacher-forced arithmetic prompt in the specified style.

    Args:
        a, b: operands
        answer_str: full answer string (e.g. "37")
        prompt_format: one of "calculate", "fewshot", "minimal", "qa", "nospace"

    Returns:
        prompt: the teacher-forced prompt (everything the model sees before target token)
    """
    ones = answer_str[-1]
    if len(answer_str) == 1:
        prefix = ""
    ...
    if prompt_format == "calculate":
        if prefix:
            return f"Calculate:\n{a} + {b} = {prefix}"
        return f"Calculate:\n{a} + {b} = "
    elif prompt_format == "fewshot":
        if prefix:
            return f"{FEWSHOT_PREFIX}{a} + {b} = {prefix}"
        return f"{FEWSHOT_PREFIX}{a} + {b} = "
...
def get_digit_sample_prompt(prompt_format: str = "calculate") -> str:
    """Return a representative sample prompt for digit token ID extraction."""
    if prompt_format == "calculate":
        return "Calculate:\n1 + 2 = "
    elif prompt_format == "fewshot":
        return f"{FEWSHOT_PREFIX}1 + 2 = "
    elif prompt_format == "minimal":
        return "1 + 2 = "
    elif prompt_format == "qa":
        return "Q: What is 1 + 2?\nA: "
    elif prompt_format == "nospace":
        return "1+2="
    else:
        raise ValueError(f"Unknown prompt_format: {prompt_format!r}")
...
VALID_PROMPT_FORMATS = ["calculate", "fewshot", "minimal", "qa", "nospace"]
...
def generate_teacher_forced_problems(
    ...
    for a in range(operand_max + 1):
        for b in range(operand_max + 1):
            ...
            prompt = format_arithmetic_prompt(a, b, answer_str, prompt_format)
...
def generate_single_digit_problems(operand_max: int = 9, prompt_format: str = "calculate") -> List[dict]:
    """
    Generate simple a+b problems where the answer is a single digit (< 10).

    This is the fallback for base models (LLaMA, GPT-2) that cannot reliably
    perform teacher-forced multi-digit arithmetic.  With operand_max=9 this
    produces ~55 problems — the same pool that fisher_patching.py v2 used
    successfully for LLaMA 3B.
    """
    problems = []
    for a in range(operand_max + 1):
        for b in range(operand_max + 1):
            ...
            if answer >= 10:
                continue
            problems.append({
                "prompt":     format_arithmetic_prompt(a, b, str(answer), prompt_format),
                "answer":     answer,
                "ones_digit": answer,
...
def generate_direct_answer_problems(
    n_per_digit: int = 100,
    operand_max: int = 99,
) -> Tuple[List[dict], Dict[int, List[dict]]]:
    """
    Generate problems for models that predict the FULL answer as a single token.

    Unlike teacher-forced problems, the prompt is just "a + b = " and the model
    predicts the entire answer (e.g., "21") in one token.  This works for
    instruct models (LLaMA 3.2-3B-Instruct) where multi-digit numbers are
    single tokens and the model cannot be teacher-forced.

    The target_str is the FULL answer string (e.g., "21"), not just the ones
    digit.  ones_digit is still answer % 10 for Fourier analysis grouping.
    """
    by_digit: Dict[int, List[dict]] = defaultdict(list)
    ...
    logger.info(f"Generated {len(flat)} direct-answer problems ({n_per_digit}/digit)")
    return flat, by_digit


def filter_correct_direct_answer(
    model,
    problems: List[dict],
    max_n: int = 200,
) -> List[dict]:
    """
    Keep only problems where the model correctly predicts the full answer
    token at the last position.  For instruct models where multi-digit
    numbers are single tokens.
    """
    correct = []
    for prob in problems:
        tokens = model.to_tokens(prob["prompt"], prepend_bos=True)
        # Get target: full answer as a single token
        target_toks = model.to_tokens(prob["target_str"], prepend_bos=False)
        if target_toks.shape[1] != 1:
            continue  # skip if answer isn't a single token
    ...
    logger.info(f"  Correct problems: {len(correct)}/{min(max_n, len(problems))} attempted")
...
def get_context_target_tok(model, prompt: str, target_str: str) -> int:
    """
    Get the token ID for target_str as it appears when following prompt.

    Tokenizes (prompt + target_str) and extracts the first new token after
    the prompt — this correctly handles BPE space-prefix behaviour across
    all tokenizers (GPT-2, LLaMA SentencePiece, Phi-3 SentencePiece, Gemma).

    Example (GPT-2):
      prompt     = "Calculate:\n3 + 5 = "   (ends with space)
      target_str = "8"
      full       = "Calculate:\n3 + 5 = 8"
      prompt has N tokens; full has N+1 tokens; last is bare "8" token ✓

    Example (multi-digit teacher-forcing):
      prompt     = "Calculate:\n13 + 8 = 2" (ends with digit, no space)
      target_str = "1"
      full       = "Calculate:\n13 + 8 = 21"
      The "1" token is the last token of full ✓
    """
    prompt_toks = model.to_tokens(prompt, prepend_bos=True)
    full_toks   = model.to_tokens(prompt + target_str, prepend_bos=True)
    n_prompt    = prompt_toks.shape[1]
    n_full      = full_toks.shape[1]

    if n_full == n_prompt + 1:
        # Normal case: target_str added exactly one token
        return full_toks[0, n_prompt].item()

    if n_full <= n_prompt:
        # BPE merged target with preceding text (e.g. "11" + "2" → "112" single token).
        # Fall back to bare tokenization — best available approximation.
        logger.debug(
            f"  [get_context_target_tok] BPE merge detected: "
            f"prompt={n_prompt} toks, full={n_full} toks. "
            f"Falling back to bare tokenization for '{target_str}'."
        )
        return model.to_tokens(target_str, prepend_bos=False)[0, -1].item()

    # target_str expanded to multiple tokens — not supported
    raise ValueError(
        f"target_str '{target_str}' tokenized to {n_full - n_prompt} tokens "
        f"(expected 1). Multi-token targets not supported."
    )
        ...
        for idx, (clean_prob, corrupt_prob) in enumerate(pairs):
            ...
            for mode in ("full", "sub", "ortho"):
                ...
                def hook_fn(act, hook, d=delta):
                    act[:, -1, :] = act[:, -1, :] + d.unsqueeze(0).to(device=act.device, dtype=act.dtype)
                    return act
...
def get_digit_token_ids(model, prompt_format: str = "calculate") -> List[int]:
    """
    ...
    """
    ids = []
    sample_prompt = get_digit_sample_prompt(prompt_format)
    for d in range(10):
...
def compute_unembed_basis(
    ...
    CENTERING: Subtract the centroid of the 10 digit vectors (mean over columns)
    so that the SVD focuses on directions that discriminate *between* digits,
    not the shared "output any digit" direction.

    W_centered = W_digits - W_digits.mean(dim=1, keepdim=True)  [dim=1 centering]

    This is standard PCA centering on 10 points in d_model-space.
    dim=1 mean = mean across the 10 digit columns = centroid of the 10 points.
    Subtracting it removes the shared direction; SVD then finds discriminative ones.
    ...
    # ── CENTERING ────────────────────────────────────────────────────────────
    # W_digits (d_model, 10): treat each column as a point in d_model-space.
    # PCA centering = subtract centroid of the 10 points = mean over columns
    # = W_digits.mean(dim=1, keepdim=True) → (d_model, 1)
    # This removes the shared "output any digit" direction so SVD finds
    # directions that DISCRIMINATE between digits (standard PCA centering).
    # dim=0 would zero-mean each column independently — NOT PCA centering.
    row_mean   = W_digits.mean(dim=1, keepdim=True)  # (d_model, 1) centroid
    W_centered = W_digits - row_mean                   # (d_model, 10)

    # Verify centering: each row should now have zero mean across the 10 digits
    post_means = W_centered.mean(dim=1).abs()
    assert post_means.max().item() < 1e-5, \
        f"[SANITY] Centering failed: max row mean = {post_means.max():.2e}"
    logger.info(
        f"  [SANITY] Row centering ✓  (max residual mean = {post_means.max():.2e})"
    )
...
def compute_unembed_basis_direct_answer(
    model,
    operand_max: int = 99,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute unembed basis for direct-answer mode.

    In direct-answer mode the model predicts full-answer tokens (e.g. "21",
    "135") rather than single-digit tokens ("0"-"9").  The standard
    compute_unembed_basis extracts W_U columns for 10 single-digit tokens,
    which span a 9D subspace.

    Direct-answer has ~199 possible answer tokens whose W_U columns span a
    ~56D effective subspace.  Group-mean-by-ones-digit fails because within-
    group cosine similarity is only ~0.25, so averaging destroys signal.

    Fix: SVD of ALL 199 answer-token W_U columns directly — no grouping.
    This captures the maximum-variance directions of the actual output space.

    Returns:
        U  : (d_model, n_answers-1) orthonormal basis — columns are directions
        S  : (n_answers-1,) singular values
    """
    max_answer = 2 * operand_max  # e.g. 99+99 = 198
    sample_prompt = "0 + 0 = "    # context for tokenization

    # ── Step 1: get token IDs for all answer values ────────────────────────
    answer_tok_ids = []
    for ans in range(max_answer + 1):
        tok_id = get_context_target_tok(model, sample_prompt, str(ans))
        answer_tok_ids.append((ans, tok_id))

    n_answers = len(answer_tok_ids)
    logger.info(
        f"  Direct-answer unembed: collected {n_answers} answer tokens "
        f"(0..{max_answer})"
    )

    # Sanity: check a few round-trips
    n_good = 0
    for ans, tok_id in answer_tok_ids:
        decoded = model.tokenizer.decode([tok_id]).strip()
        if decoded == str(ans):
            n_good += 1
    logger.info(
        f"  [SANITY] {n_good}/{n_answers} answer tokens round-trip correctly"
    )

    # ── Step 2: extract W_U columns for ALL answer tokens ─────────────────
    W_U = model.W_U.detach().float().cpu()   # (d_model, vocab_size)
    d_model = W_U.shape[0]

    tok_ids = [tid for _, tid in answer_tok_ids]
    W_answers = W_U[:, tok_ids]  # (d_model, n_answers)

    # ── Step 3: center and SVD ─────────────────────────────────────────────
    row_mean   = W_answers.mean(dim=1, keepdim=True)  # (d_model, 1)
    W_centered = W_answers - row_mean                   # (d_model, n_answers)
    ...
    # Verify orthonormality (check first 50 columns)
    n_check = min(50, U.shape[1])
    G = (U[:, :n_check].T @ U[:, :n_check]).numpy()
    off_diag_err = np.abs(G - np.eye(n_check)).max()
    assert off_diag_err < 1e-4, \
        f"[SANITY] U not orthonormal: max off-diag = {off_diag_err:.2e}"
    logger.info(
        f"  [SANITY] SVD U columns orthonormal ✓  (max err = {off_diag_err:.2e})"
    ...
    S2_norm = (S_np ** 2) / var_all
    eff_dim = 1.0 / (S2_norm ** 2).sum()

    logger.info(f"  Top-10 singular values: {S_np[:10].round(3)}")
    logger.info(f"  Effective dimensionality: {eff_dim:.1f}")
    for k in [1, 2, 5, 9, 10, 20, 50]:
        pct = 100 * (S_np[:k] ** 2).sum() / var_all
        logger.info(f"  Top-{k:2d} explain {pct:.1f}% of answer-token W_U variance")
...
def main():
    parser = argparse.ArgumentParser(
        description="Arithmetic Circuit Scan + Unembed Patching (v2 — multi-model)"
    )
    ...
    # ── Safety: MPS has broken gradients; only block when gradients are needed ──
    device = args.device
    ...
    if device == "mps" and needs_gradients:
        ...
        logger.info("MPS safe for this run (no gradient experiments selected).")
    ...
    if args.direct_answer:
        test_flat, _ = generate_direct_answer_problems(n_per_digit=15, operand_max=99)
        correct = filter_correct_direct_answer(model, test_flat, max_n=args.n_test)
        logger.info(f"  Direct-answer correct: {len(correct)}")
    else:
        ...
        if len(correct) < 20:
            logger.warning(
                f"  Only {len(correct)} correct problems from teacher-forced set. "
                "Falling back to single-digit-only problems (a+b < 10)."
            )
            single_probs = generate_single_digit_problems(operand_max=9)
            correct = filter_correct_teacher_forced(model, single_probs, max_n=args.n_test)
            logger.info(f"  Single-digit fallback: {len(correct)} correct problems")

    all_results["n_correct_test"] = len(correct)

    assert len(correct) >= 20, (
        f"[SANITY] Only {len(correct)} correct problems even after fallback. "
        "Model appears unable to handle arithmetic at all. "
        "Check prompt format or try a different model."
    )
    ...
    if not args.no_fisher:
        ...
        # Generate balanced Fisher problems (100/digit)
        if args.direct_answer:
            fisher_flat, fisher_by_digit = generate_direct_answer_problems(
    ...
    if not args.no_unembed:
        ...
        for layer in unembed_layers:
            ...
            if not args.no_random_control:
                ...
                R_basis = random_orthonormal_basis(d_model, n_dims=max_rand_dim, seed=42)
    ...
    if not args.no_format_removal:
        # Note: format removal is designed for instruction-tuned models.
        # For base models (gpt2, llama-3b), formatting Fisher is mostly noise
        # since these models have no instruction-following subspace.
        # The experiment still runs but expect minimal subspace overlap.
        ...
        for layer in unembed_layers:
            ...
            results_cleaned = run_patching_experiment(
                ...
                n_dims_list=[5, 10, 20, 50], label="ArithClean",
                verify_partition_on_first=True,
                direct_answer=args.direct_answer,
            )
    ...
    out_path = out_dir / f"arithmetic_scan_{model_slug}{suffix}.json"
