"""Stage 1 (Gemma variant): extract (stereo, anti-stereo) probe pairs
from StereoSet intrasentence using the Gemma tokenizer.

Mirror of experiments/stereoset_probe_extract.py but tokenizes fills
with Gemma-2-2b's SentencePiece tokenizer. Retains pairs where both
sides are single-token (tried with and without leading space).

Outputs:
  helix_usage_validated/stereoset_probe_pairs_gemma.tsv
  helix_usage_validated/stereoset_probe_halfmatch_gemma.tsv
"""
...
def tokenize_single(tok, word: str) -> tuple[int | None, str | None]:
    ...
    one-token under the Gemma tokenizer. Otherwise (None, None)."""
...
def main():
    print(f"Loading StereoSet intrasentence ...")
    ...
    print()
    print("=== per-domain summary (Gemma tokenizer) ===")
    for d in DOMAINS:
    ...
    pair_path = OUT_DIR / "stereoset_probe_pairs_gemma.tsv"
    ...
    half_path = OUT_DIR / "stereoset_probe_halfmatch_gemma.tsv"
