"""Stage 2 (Gemma variant): scan Gemma-2-2b's top-4 OV-SVD planes
against every Gemma-tokenized StereoSet probe pair from Stage 1.

Same two-stage filter as the GPT-2 version:
  2a) Decode pre-filter via top-K vocabulary overlap per (L, H, plane).
  2b) Causal alpha-sweep on surviving candidates only.

Output:
  helix_usage_validated/stereoset_scan_gemma.jsonl
  helix_usage_validated/stereoset_scan_gemma_summary.txt
"""
...
PAIRS_PATH = OUT_DIR / "stereoset_probe_pairs_gemma.tsv"
...
def main():
    ...
    jsonl_path = OUT_DIR / "stereoset_scan_gemma.jsonl"
    ...
    sum_path = OUT_DIR / "stereoset_scan_gemma_summary.txt"
