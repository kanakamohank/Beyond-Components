...
Fixes over v1:
  BUG-1  compute_unembed_basis: centering kept at dim=1 (discriminative, correct).
...
def get_digit_token_ids(model) -> List[int]:
    """Get token IDs for the digit characters 0–9."""
    ids = []
    for d in range(10):
        toks = model.to_tokens(str(d), prepend_bos=False)
        ids.append(toks[0, -1].item())
    logger.info(f"  Digit token IDs: {ids}")

    # Sanity: all IDs should be distinct
...
def compute_unembed_basis(
    ...
    CENTERING: We subtract the mean digit vector (mean across the 10 columns
    for each residual-stream dimension) so that SVD focuses on directions that
    *discriminate between digits*, not the shared "output any digit" direction.

    W_centered = W_digits - mean_digit_vector   (dim=1 centering)

    This is analogous to PCA centering: subtract the mean observation.
    v1 originally used dim=1 which was correct; the initial "BUG-1" diagnosis
    was wrong — dim=0 (scalar column means) doesn't remove the shared direction.
    ...
    # Centering over dim=1 removes the mean digit vector (shared direction)
    # so SVD finds directions that discriminate between digits.
    row_mean   = W_digits.mean(dim=1, keepdim=True)  # (d_model, 1) — mean digit vec
    W_centered = W_digits - row_mean                   # (d_model, 10)

    # Verify centering correctness: each row should have zero mean across digits
...
def compute_fisher_matrix(
    ...
    for i, prob in enumerate(problems[:n]):
        ...
        target_tok = model.to_tokens(
            prob["target_str"], prepend_bos=False
        )[0, -1].item()
...
def main():
    ...
    parser.add_argument("--no-unembed",       action="store_true",
                        help="Skip unembed-aligned patching (Exp 2)")
    ...
    if device == "mps" and needs_gradients:
        logger.warning(
            ...
            "Forcing device=cpu for gradient-based experiments."
        )
        device = "cpu"
    elif device == "mps":
        logger.info("MPS safe: no gradient experiments enabled (Fisher/format-removal disabled).")

    log_section("PHI-3 ARITHMETIC LAYER SCAN + UNEMBED PATCHING (v2)")
    ...
    log_section("UNEMBED BASIS (centering=dim=1, discriminative SVD)")
    ...
    for layer in unembed_layers if not args.no_unembed else []:
        logger.info(f"\n  Layer {layer}")
        ...
        if not args.no_random_control:
            ...
            for nd in n_dims_list:
                ...
                if key in results_unembed and key in results_random:
                    ...
                    # Soft warning, not hard assert (unembed could be genuinely bad)
    ...
    out_path = out_dir / "phi3_layer_scan_unembed_v2.json"
