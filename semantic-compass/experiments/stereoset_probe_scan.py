"""Stage 2: scan GPT-2 Small's top-4 OV-SVD planes against every
StereoSet probe pair from Stage 1.

Two-stage filter:
  2a) Decode pre-filter. For each (L, H, plane=(d1,d2)) project the
      SVD basis through W_U and flag probe pairs (t+, t-) where each
      probe token lands in the top-k of either u_{d1} or u_{d2} pole.
      This is a cheap matmul with no forward passes and prunes the
      (plane, probe) combinations where the plane cannot possibly
      read the probe's vocabulary axis.

  2b) Causal sweep. Run the alpha-sweep only on (plane, probe) pairs
      surviving 2a. Per (L, H, plane, alpha, theta, prompt) we do ONE
      forward pass and cache the full final-token logits; candidate
      probes are scored by vocabulary lookup.

Output:
  helix_usage_validated/stereoset_scan_gpt2.jsonl
    (one row per (L, H, d1, d2, probe_pair, domain))
  helix_usage_validated/stereoset_scan_gpt2_summary.txt
    (per-head pass counts aggregated across probes)
"""
...
PAIRS_PATH = OUT_DIR / "stereoset_probe_pairs.tsv"
...
# decode pre-filter
DECODE_TOPK = 500  # how deep to scan each pole's vocabulary for probe hits
...
def decode_topk_ids(W_U_T, u, k):
    """Top-k token ids for u and -u under W_U. W_U_T is [d_model, V]."""
...
def main():
    ...
    print(f"[{time.time()-t_start:6.1f}s] Loading gpt2 on {device} ...",
          flush=True)
    ...
    # W_U: [d_model, V] in HookedTransformer
    W_U = model.W_U.detach().float().cpu()  # [d_model, V]

    prompts_toks = [model.to_tokens(p) for p in NEUTRAL_PROMPTS]
    angles = np.linspace(0, 360, N_ANGLES, endpoint=False)
    sv_pairs = list(itertools.combinations(range(TOP_SVS), 2))

    # ---- Stage 2a: decode pre-filter ----
    # For each (L, H) compute top-4 SVs; for each SV dim, cache
    # top-k ids for +u and -u. A probe pair (s_id, a_id) is
    # candidate for plane (d1, d2) if:
    #   (s_id in pos_d1 and a_id in neg_d1) OR
    #   (a_id in pos_d1 and s_id in neg_d1) OR
    #   same for d2.
    print(f"[{time.time()-t_start:6.1f}s] Stage 2a: decode pre-filter ...",
          flush=True)
    candidate_map = {}  # (L, H, d1, d2) -> np.array of probe indices
    ...
    for L in range(n_layers):
        for H in range(n_heads):
            ...
            for d in range(top):
                ...
                pos_sets.append(pos)
                neg_sets.append(neg)
            # Per plane: find probes where at least one axis has
    ...
    print(f"[{time.time()-t_start:6.1f}s]   (plane,probe) candidates: "
          ...
          flush=True)
    # Show top heads by candidate count
    ...
    # ---- Stage 2b: causal sweep on candidates only ----
    # For each plane that has any candidates, do the alpha-sweep
    # (caching final-token logits) and score those probes.
    ...
    for L in range(n_layers):
        ...
        for H in heads_in_layer:
            ...
            for (d1, d2) in sv_pairs:
                ...
                Pc = len(cand_idx)
                probe_ids = all_probe_ids[cand_idx]  # [Pc, 2]
    ...
    jsonl_path = OUT_DIR / "stereoset_scan_gpt2.jsonl"
    ...
    sum_path = OUT_DIR / "stereoset_scan_gpt2_summary.txt"
    with sum_path.open("w") as f:
        f.write("StereoSet multi-probe scan summary (GPT-2 Small)\n")
