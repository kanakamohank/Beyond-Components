...
def plot_pca_variance_scree(model_name="google/gemma-7b", layer=21):
    print(f"\n📉 Generating PCA Scree Plot for Dimensionality Check at Layer {layer}...")
    ...
    for i, prompt in enumerate(prompts):
        ...
        if target_pos == -1:
            raise ValueError(f"CRASH PREVENTED: Could not align token for digit {digits[i]}")

        _, cache = model.run_with_cache(tokens, names_filter=hook_name)
        live_vectors.append(cache[hook_name][0, target_pos, :].detach().float().cpu().numpy())

    V = np.array(live_vectors)
    ...
    bars = ax1.bar(range(1, 10), variance_explained, alpha=0.7, color='teal', label='Individual Variance')
    ax1.set_xlabel('Principal Component', fontweight='bold')
    ax1.set_ylabel('Variance Explained (%)', fontweight='bold', color='teal')
    ax1.tick_params(axis='y', labelcolor='teal')
    ax1.set_xticks(range(1, 10))

    ax2 = ax1.twinx()
    ax2.plot(range(1, 10), cumulative_variance, marker='o', color='darkorange', linewidth=2, label='Cumulative Variance')
    ax2.set_ylabel('Cumulative Variance (%)', fontweight='bold', color='darkorange')
    ax2.tick_params(axis='y', labelcolor='darkorange')
    ax2.axhline(90, color='red', linestyle='--', alpha=0.5, label='90% Threshold')

    plt.title(f'PCA Scree Plot: Arithmetic Dimensionality\n{model_name} (Layer {layer})', fontsize=14, fontweight='bold')
    fig.tight_layout()

    clean_model_name = model_name.split('/')[-1]
    save_path = f"{clean_model_name}_scree_plot_layer_{layer}.png"
    plt.savefig(save_path, dpi=300)
